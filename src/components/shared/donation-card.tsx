  ...
  return (
    <Card className="flex flex-col h-full shadow-lg hover:shadow-xl transition-shadow duration-300">
      ...
      <CardContent className="p-4 flex-grow">
        <CardTitle className="text-lg font-bold mb-2">{donation.title}</CardTitle>
        ...
        <div className="flex items-center text-sm text-muted-foreground">
          ...
          <span>Expires {formatDistanceToNow(donation.expiry, { addSuffix: true })}</span>
        </div>
      </CardContent>
      <CardFooter className="p-4 flex justify-between items-center bg-secondary/50 rounded-b-lg">
        <div className="font-semibold text-foreground">{donation.quantity}</div>
